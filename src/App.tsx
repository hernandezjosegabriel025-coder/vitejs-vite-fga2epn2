...
function recalcularDesde(pagos: typeof PAGOS_GABRIEL_BASE, desdeNum: number, nuevoSaldo: number) {
...
const PRESTAMO_GABRIEL = {
  ...
  pagos: PAGOS_GABRIEL_BASE,
  color: "#0ea5e9",
  emoji: "🏦",
  precargado: true,
};

const mxn = (n: number) => new Intl.NumberFormat("es-MX",{style:"currency",currency:"MXN",maximumFractionDigits:2}).format(n||0);
const todayStr = () => new Date().toISOString().split("T")[0];
const COLORES = ["#0ea5e9","#a78bfa","#34d399","#fb923c","#f472b6","#facc15","#60a5fa","#f87171"];
const EMOJIS = ["🏦","🏠","🚗","💳","📱","🎓","💼","🏥"];

function generarAmortizacion(capital: number, tasaAnual: number, totalPagos: number, frecuencia: string, fechaInicio: string) {
...
const Badge = ({children, color}: {children: React.ReactNode, color: string}) => (
  <span style={{display:"inline-block",padding:"2px 8px",borderRadius:20,fontSize:10,fontWeight:700,letterSpacing:0.5,background:color+"22",color}}>{children}</span>
);

type Prestamo = {
  id: string; nombre: string; titular: string; capital: number; tasaAnual: number;
  años: number; formaPago: string; totalPagos: number; fechaInicio: string; fechaFin: string;
  totalIntereses: number; iva: number; costoTotal: number;
  pagos: typeof PAGOS_GABRIEL_BASE; color: string; emoji: string; precargado?: boolean;
};
...
export default function App() {
  ...
  const [pagadosMap, setPagadosMap] = useState<Record<string,number[]>>(() => {
    try { return JSON.parse(localStorage.getItem("pagados_v2")||"{}"); } catch { return {}; }
  });
  const [abonosMap, setAbonosMap] = useState<Record<string,Record<number,number>>>(() => {
    try { return JSON.parse(localStorage.getItem("abonos_v2")||"{}"); } catch { return {}; }
  });
  const [pagosMap, setPagosMap] = useState<Record<string, typeof PAGOS_GABRIEL_BASE>>(() => {
    try { return JSON.parse(localStorage.getItem("pagos_recalc_v2")||"{}"); } catch { return {}; }
  });

  const [pantalla, setPantalla] = useState("inicio");
  const [prestamoActivo, setPrestamoActivo] = useState<string|null>(null);
  const [tab, setTab] = useState("resumen");
  const [liquidarNum, setLiquidarNum] = useState<number|null>(null);
  const [showAll, setShowAll] = useState(false);
  const [abonoNum, setAbonoNum] = useState<number|null>(null);
  const [abonoValor, setAbonoValor] = useState("");

  const [form, setForm] = useState({
    nombre:"", titular:"", capital:"", tasaAnual:"", años:"",
    formaPago:"mensual", fechaInicio:todayStr(), color:COLORES[1], emoji:EMOJIS[0]
  });
  ...
  const getPagados = (id: string) => pagadosMap[id]||[];
  const getPagos = (p: Prestamo) => pagosMap[p.id]||p.pagos;

  const togglePago = (id: string, num: number) => {
    setPagadosMap(m => {
      const prev = m[id]||[];
  ...
  const aplicarAbono = (prestamo: Prestamo, num: number, monto: number) => {
  ...
  const totalDeuda = prestamos.reduce((s,p) => {
    ...
    const sig = getPagos(p).find(x => !pags.includes(x.num));
  ...
  const agregarPrestamo = () => {
    ...
    const nuevo: Prestamo = {
      id:"prestamo-"+Date.now(), nombre:form.nombre, titular:form.titular||"Sin titular",
      capital:+form.capital, tasaAnual:+form.tasaAnual, años:+form.años, formaPago:form.formaPago,
  ...
  const abrirDetalle = (id: string) => {
    setPrestamoActivo(id); setTab("resumen"); setLiquidarNum(null); setShowAll(false); setPantalla("detalle");
  };
    ...
    <div style={{minHeight:"100vh",background:"#060810",color:"#f0f2ff",fontFamily:"'DM Sans',sans-serif",maxWidth:430,margin:"0 auto",paddingBottom:80}}>
      ...
      <div style={{background:"linear-gradient(160deg,#0a1220,#060810)",padding:"40px 20px 24px",borderBottom:"1px solid #0f1520"}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:18}}>
          {[{l:"Deuda total",v:mxn(totalDeuda),c:"#f87171"},{l:"Próx. pagos",v:mxn(totalPagosProximos),c:"#fbbf24"},{l:"Préstamos",v:String(prestamos.length),c:"#34d399"}].map(({l,v,c})=>(
    ...
    <div style={{minHeight:"100vh",background:"#060810",color:"#f0f2ff",fontFamily:"'DM Sans',sans-serif",maxWidth:430,margin:"0 auto",paddingBottom:40}}>
      ...
      <div style={{padding:"20px 16px"}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          {EMOJIS.map(e=>(
            <button key={e} onClick={()=>setForm(f=>({...f,emoji:e}))} style={{width:40,height:40,borderRadius:10,border:`2px solid ${form.emoji===e?"#0ea5e9":"#1e293b"}`,background:form.emoji===e?"#0ea5e922":"#0d1117",fontSize:20,cursor:"pointer"}}>{e}</button>
          ))}
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <div><div className="label-xs" style={{marginBottom:6}}>Capital (MXN) *</div>
            <input className="input" type="number" placeholder="0" value={form.capital} onChange={e=>setForm(f=>({...f,capital:e.target.value}))}/></div>
          <div><div className="label-xs" style={{marginBottom:6}}>Tasa anual % *</div>
            <input className="input" type="number" placeholder="0" value={form.tasaAnual} onChange={e=>setForm(f=>({...f,tasaAnual:e.target.value}))}/></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <div><div className="label-xs" style={{marginBottom:6}}>Plazo (años) *</div>
            <input className="input" type="number" placeholder="1" value={form.años} onChange={e=>setForm(f=>({...f,años:e.target.value}))}/></div>
  ...
  if (pantalla==="detalle" && prestamo) {
    ...
    return (
      <div style={{minHeight:"100vh",background:"#060810",color:"#f0f2ff",fontFamily:"'DM Sans',sans-serif",maxWidth:430,margin:"0 auto",paddingBottom:30}}>
        ...
        <div style={{background:`linear-gradient(160deg,#0a1220,#060810)`,padding:"36px 20px 20px",borderBottom:"1px solid #111827",borderTop:`3px solid ${ac}`}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
            ...
            <button className="btn btn-ghost" style={{marginLeft:"auto",padding:"6px 10px",fontSize:16,color:"#ef4444"}}
              onClick={()=>{ if(window.confirm("¿Eliminar?")) eliminarPrestamo(prestamo.id); }}>🗑</button>
          </div>
        ...
        {tab==="resumen" && (
          <div style={{padding:"14px 16px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {[
                ["Pagos hechos",String(pagados.length),"#34d399"],
                ["Pagos restantes",String(prestamo.totalPagos-pagados.length),"#fbbf24"],
            ...
            <div style={{marginTop:14,background:"#0a1220",borderRadius:14,padding:14,border:`1px solid ${ac}33`}}>
              ...
              <div style={{fontSize:14,color:"#9ca3af",lineHeight:1.7}}>
                Saldo a liquidar: <span style={{color:ac,fontWeight:800}} className="mono">{mxn(siguientePago?.saldo||0)}</span>
              </div>
        ...
        {tab==="tabla" && (
          <div>
            <div style={{padding:"10px 16px 4px",fontSize:12,color:"#4b5563"}}>Toca para marcar pagado · 💰 para abonar a capital</div>
            {pagosVis.map(p=>{
                ...
                <div key={p.num}>
                  <div className="pago-row" style={{opacity:isPag?0.55:1}}>
                    <div className="check" style={isPag?{background:"#15803d",borderColor:"#15803d"}:esProx?{borderColor:ac}:{}}
                      onClick={()=>togglePago(prestamo.id,p.num)}>
                    ...
                    <div style={{textAlign:"right",display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                      ...
                      {!isPag && (
                        <button onClick={()=>setAbonoNum(abonoNum===p.num?null:p.num)}
                          style={{fontSize:11,padding:"3px 8px",borderRadius:8,border:`1px solid #a78bfa44`,background:"#a78bfa11",color:"#a78bfa",cursor:"pointer"}}>
                          💰 Abonar
                        </button>
                      )}
                    </div>
                  ...
                  {abonoNum===p.num && (
                    <div style={{background:"#0a0d14",padding:"12px 16px",borderBottom:"1px solid #0a0f18"}}>
                      ...
                      <div style={{fontSize:12,color:"#6b7280",marginBottom:8}}>
                        Saldo actual: <span style={{color:"#f0f2ff",fontWeight:700}} className="mono">{mxn(p.saldo)}</span>
                      </div>
                      <input
                        className="input" type="number" placeholder="Monto a abonar"
                        value={abonoValor} onChange={e=>setAbonoValor(e.target.value)}
                      ...
                      {abonoValor && +abonoValor > 0 && (
                        <div style={{background:"#060810",borderRadius:10,padding:"10px 12px",marginBottom:10,fontSize:12,color:"#6b7280",lineHeight:1.8}}>
                          Nuevo saldo: <span style={{color:"#a78bfa",fontWeight:800}} className="mono">{mxn(Math.max(0,p.saldo - +abonoValor))}</span><br/>
                          Los siguientes pagos se recalcularán automáticamente
                        </div>
                      )}
                      <div style={{display:"flex",gap:8}}>
                        <button className="btn btn-ghost" style={{flex:1,padding:"10px"}} onClick={()=>{setAbonoNum(null);setAbonoValor("");}}>Cancelar</button>
                        <button className="btn btn-primary" style={{flex:2,padding:"10px",background:"linear-gradient(135deg,#6d28d9,#a78bfa)"}}
                          onClick={()=>aplicarAbono(prestamo,p.num,+abonoValor)}
                          disabled={!abonoValor||+abonoValor<=0}>
                          ✓ Aplicar abono
                        </button>
                      </div>
                    </div>
